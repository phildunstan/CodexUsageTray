...
- **Dynamic icon:** the usage ring is drawn in memory with GDI+. Its unused segment is blue when any task is active, grey when idle, and red when less than 20% remains; the used segment is light grey. The exact percentage remains available in the tooltip. If the API is unavailable, the last successful percentage is retained and shown in grey.
